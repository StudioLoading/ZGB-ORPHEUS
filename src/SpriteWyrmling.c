...
const UINT8 a_wyrmling_hidden[] = {1, 0};
const UINT8 a_wyrmling_up[] = {4, 1,2,3,2};
const UINT8 a_wyrmling_down[] = {4, 1,2,3,2};
const UINT8 a_wyrmling_h[] = {4, 1,2,3,2};
const UINT8 a_wyrmling_repelled[] = {2, 0,4};
const UINT8 a_wyrmling_attack[] = {1, 4};
...
void wyrmling_update_anim(Sprite* s_enemy, SPRITE_STATES new_state) BANKED;


void START(){
    SetSpriteAnim(THIS, a_wyrmling_hidden, 6);
...
void wyrmling_update_anim(Sprite* s_enemy, SPRITE_STATES new_state) BANKED{
    switch(new_state){
        case IDLE_UP: case WALK_UP: 
            SetSpriteAnim(s_enemy, a_wyrmling_up, 20);
        break;
        case IDLE_DOWN: case WALK_DOWN:
            SetSpriteAnim(s_enemy, a_wyrmling_down, 4);
        break;
        case IDLE_LEFT: case WALK_LEFT:
            s_enemy->mirror = NO_MIRROR;
            SetSpriteAnim(s_enemy, a_wyrmling_h, 16);
        break;
        ...
        break;
        case HIT:
            SetSpriteAnim(s_enemy, a_wyrmling_repelled, 16);
        break;
        case ATTACK:
            SetSpriteAnim(s_enemy, a_wyrmling_attack, 8);
        break;
    }
}
