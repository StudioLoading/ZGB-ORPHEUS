...
void lostsoul_update_anim(Sprite* s_enemy, SPRITE_STATES new_state) BANKED{
    switch(new_state){
        case IDLE_UP: case WALK_UP: 
            SetSpriteAnim(s_enemy, a_lostsoul_up, 6);
        break;
        case IDLE_DOWN: case WALK_DOWN:
            SetSpriteAnim(s_enemy, a_lostsoul_down, 6);
        break;
        case IDLE_LEFT: case WALK_LEFT:
            s_enemy->mirror = V_MIRROR;
            SetSpriteAnim(s_enemy, a_lostsoul_h, 8);
        break;
        ...
        break;
        case HIT:
            SetSpriteAnim(s_enemy, a_lostsoul_repelled, 12);
        break;
    }
}
