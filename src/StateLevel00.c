...
void START() {
	...
	//SPRITES
		...
		SpriteManagerAdd(SpriteBlock, ((UINT16) 30u << 3), ((UINT16) 23u << 3) + 2u);
		s_door = SpriteManagerAdd(SpriteBlock, ((UINT16) 29u << 3), ((UINT16) 6u << 3));
		//ENEMIES
		/*
	...
	//BLOCK MANAGEMENT
		if(s_orpheus->y < ((UINT16) 28u << 3)){
			if(init_block_button == 0){
				s_block = SpriteManagerAdd(SpriteBlock, ((UINT16) 25u << 3), ((UINT16) 9u << 3) + 2u);
				init_block_button = 1u;
			}
			UINT8 tile = GetScrollTile((s_block->x + 8) >> 3, (s_block->y+8) >> 3);
			...
			if(init_block_button == 1u){
				if(button_pressed == 0){
					s_block->x = ((UINT16) 25u << 3);
					s_block->y = ((UINT16) 9u << 3) + 2u;
				}
			}
		}
}
