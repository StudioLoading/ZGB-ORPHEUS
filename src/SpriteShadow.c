...
const UINT8 a_shadow_hidden[] = {1, 0};
const UINT8 a_shadow_up[] = {2, 1, 2};
const UINT8 a_shadow_down[] = {2, 3, 4};
const UINT8 a_shadow_h[] = {2, 5, 6};
const UINT8 a_shadow_repelled[] = {4, 1,5,3,6};
...
void shadow_update_anim(Sprite* s_enemy, SPRITE_STATES new_state) BANKED;


void START(){
    SetSpriteAnim(THIS, a_shadow_hidden, 6);
...
void shadow_update_anim(Sprite* s_enemy, SPRITE_STATES new_state) BANKED{
    switch(new_state){
        case IDLE_UP: case WALK_UP: 
            SetSpriteAnim(s_enemy, a_shadow_up, 6);
        break;
        case IDLE_DOWN: case WALK_DOWN:
            SetSpriteAnim(s_enemy, a_shadow_down, 6);
        break;
        case IDLE_LEFT: case WALK_LEFT:
            s_enemy->mirror = V_MIRROR;
            SetSpriteAnim(s_enemy, a_shadow_h, 8);
        break;
        ...
        break;
        case HIT:
            SetSpriteAnim(s_enemy, a_shadow_repelled, 12);
        break;
    }
}
